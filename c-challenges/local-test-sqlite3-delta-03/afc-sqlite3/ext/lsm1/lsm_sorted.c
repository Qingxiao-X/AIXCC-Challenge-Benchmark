...
/*
** Flush and then view the internal representation of the database.
** Useful for taking a look from within SQLite.
*/
LsmString* lsm1ViewInternal(lsm_db* pDb){
  LsmString* ret = 0;
  int rc = lsmBeginWork(pDb);
  if ( rc == LSM_OK ){
    ret = lsmSortedDumpStructure(pDb, pDb->pWorker, LSM_LOG_DATA, 0, "view");
    lsmFinishWork(pDb, 1, &rc);
  }else{
    lsmLogMessage(pDb, rc, "ERROR: Could not flush while viewing");
  }
  return ret;
}

int lsm1InternalFlush(lsm_db* pDb){
  return lsmFlushTreeToDisk(pDb);
}
...
void sortedDumpSegment(LsmString* s, lsm_db *pDb, Segment *pRun, int bVals){
  assert( pDb->xLog );
  if( pRun && pRun->iFirst ){
    ...
    lsmStringAppendf(s, "Segment: %s", zSeg);
    lsmFree(pDb->pEnv, zSeg);
...
*/
LsmString* lsmSortedDumpStructure(
  ...
  Level *pTopLevel;
  char *zFree = 0;
  LsmString* s = lsmMallocZero(pDb->pEnv, sizeof(LsmString));
  if(!s) return 0;
  lsmStringInit(s, pDb->pEnv);
  ...
  if( pDb->xLog && pTopLevel ){
    ...
    nCall++;
    lsmStringAppendf(s, "Database structure %d (%s)", nCall, zWhy);
    ...
    for(pLevel=pTopLevel; pLevel; pLevel=pLevel->pNext){
      ...
      for(i=0; i<nLeft || i<nRight; i++){
        ...
        lsmStringAppendf(s, "% 25s % *s% -35s %s", 
    ...
    if( bKeys ){
      for(pLevel=pTopLevel; pLevel; pLevel=pLevel->pNext){
        int i;
        sortedDumpSegment(s, pDb, &pLevel->lhs, bVals);
        for(i=0; i<pLevel->nRight; i++){
          sortedDumpSegment(s, pDb, &pLevel->aRhs[i], bVals);
        }
      }
    }
  }
  ...
  lsmStringAppendf(s, "Freelist: %s", zFree);
