...
/*
** Immediately view the internal representation of the database.
** Useful for taking a look from within SQLite.
*/
void lsm1FunctionView(
  sqlite3_context *ctx,
  int argc,
  sqlite3_value **argv
){
  lsm1_vtab* vtab = (lsm1_vtab*)sqlite3_user_data(ctx);
  LsmString* ret = lsm1ViewInternal(vtab->pDb);
  if( !ret ) {
    sqlite3_result_error_nomem(ctx);
  }else{
    sqlite3_result_text(ctx, ret->z, -1, SQLITE_TRANSIENT);
    lsmStringClear(ret);
    lsm_free(vtab->pDb->pEnv, ret);
  }
}

/*
** This function allows the user to merge all sorted runs
** and flush them out to disk right away. Useful for cleaning
** for performance reasons.
*/
void lsm1FunctionFlush(
  sqlite3_context *ctx,
  int argc,
  sqlite3_value **argv
){
...
static int lsm1FindFunction(
  ...
  (void)nArg;
  if( sqlite3_stricmp("lsm_view", zName)==0 ){
    *pxFunc = lsm1FunctionView;
    *ppArg = (void*)pVtab;
    return 1;
  }
  if( sqlite3_stricmp("lsm_flush", zName)==0 ){
    *pxFunc = lsm1FunctionFlush;
    *ppArg = (void*)pVtab;
    return 1;
  }
  return 0;
...
int sqlite3LsmInit(sqlite3* db){
  ...
  if (rc==SQLITE_OK) rc = sqlite3_overload_function(db, "lsm_view", -1);
  if (rc==SQLITE_OK) rc = sqlite3_overload_function(db, "lsm_flush", -1);
  return rc;
}
